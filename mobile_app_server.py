...
def detect_objects_endpoint():
    ...
    while not STOP_WEBCAM_FLAG:
        ...
        if initial_capture or (current_time - start_time >= interval):
            ...
            prediction = model.predict(source=frame, conf=0.7, iou=0.4)

            formatted_json = save_predictions_to_json(
                output_dir, timestamp_replaced, timestamp, prediction, all_products
            )
            detected_objects_frame = save_detected_objects_frame(
                output_dir, timestamp_replaced, prediction[0].plot()
            )
            all_products_count = save_product_counts_to_json(
                all_products, threshold, prediction
            )
            save_current_frame("current_image.jpg", prediction[0].plot())
            update_status_and_upload_to_dynamodb(
                dynamodb,
                TABLE_NAME,
                all_products,
        ...
        cv2.imshow("CCTV", prediction[0].plot())
